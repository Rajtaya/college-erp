...
router.get('/students/:subject_id', verify('teacher', 'admin'), async (req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT s.student_id, TRIM(CONCAT(IFNULL(s.first_name,''), ' ', IFNULL(s.last_name,''))) AS name, s.roll_no
...
router.get('/subjects/:student_id', async (req, res) => {
  ...
  try {
    ...
    // UG: MAJOR from enrollment (admin-imported), non-MAJOR from subjects table
    let subjects;
    if (isPG) {
      const [rows] = await db.query(
      ...
      // ── UG Enrollment: Two separate queries ─────────────────────────────
      //
      // MAJOR: Always from student_subject_enrollment (admin imports these)
      //   - Admin bulk-imports MAJOR subjects per student
      //   - Admin can also add/edit MAJOR via manual student form
      //   - If no MAJOR imported yet, section is empty (correct behavior)
      //
      // Non-MAJOR (MDC, MIC, VAC, AEC, SEC): From subjects table
      //   - is_common=1, filtered by level
      //   - MDC/MIC exclude student's MAJOR disciplines
      //   - Student selects from these pools

      // Query 1: MAJOR from enrollment (admin-assigned)
      const [majorRows] = await db.query(
        `SELECT s.subject_id, s.subject_code, s.subject_name, s.category,
                s.semester, s.credits, s.internal_marks,
                s.level_id, s.programme_id, s.faculty_id,
                s.discipline_id, s.is_common, d.discipline_name,
         ...
         LEFT JOIN disciplines d ON s.discipline_id = d.discipline_id
         WHERE e.student_id = ? AND s.category = 'MAJOR' AND s.semester = ?
         ORDER BY d.discipline_name, s.subject_code`,
        [req.params.student_id, student.semester]
      );

      // Query 2: Non-MAJOR from subjects table (student selects)
      const [nonMajorRows] = await db.query(
        `SELECT s.subject_id, s.subject_code, s.subject_name, s.category,
                s.semester, s.credits, s.internal_marks,
                s.level_id, s.programme_id, s.faculty_id,
                s.discipline_id, s.is_common, d.discipline_name,
         ...
         LEFT JOIN student_subject_enrollment e
           ON s.subject_id = e.subject_id AND e.student_id = ?
         WHERE s.semester = ? AND s.category != 'MAJOR'
           AND s.is_common = TRUE AND s.level_id = ?
           AND (
             s.category IN ('VAC', 'AEC', 'SEC')
             OR (s.category = 'MDC' AND (? = 0 OR s.discipline_id NOT IN (?)))
             OR (s.category = 'MIC' AND (? = 0 OR s.discipline_id NOT IN (?)))
             OR (s.category NOT IN ('VAC','AEC','SEC','MDC','MIC'))
           )
         ORDER BY s.category, s.subject_code`,
        [req.params.student_id, student.semester,
         student.level_id,
      ...
      subjects = [...majorRows, ...nonMajorRows];
    }
...
router.post('/save-draft/:student_id', verify('student', 'admin'), async (req, res) => {
  ...
  try {
    ...
    if (decisions.length > 0) {
      // Batch INSERT — include semester + academic_year_id for the CHECK constraint
      ...
      const values = decisions.flatMap(d => [
        ...
        student.semester, student.academic_year_id || null
      ]);
await db.query(
        `INSERT INTO student_subject_enrollment
           (student_id, subject_id, status, is_major, remarks, is_draft, semester, academic_year_id)
         VALUES ${placeholders}
         ON DUPLICATE KEY UPDATE
           status   = IF(admin_modified = 1, status,   VALUES(status)),
           is_major = IF(admin_modified = 1, is_major, VALUES(is_major)),
           remarks  = IF(admin_modified = 1, remarks,  VALUES(remarks)),
           is_draft = IF(admin_modified = 1, is_draft, 1)`,
        values
      );
    }
    res.json({ message: 'Draft saved successfully' });
  } catch (err) { console.error('SAVE_DRAFT ERROR:', err); res.status(500).json({ error: err.message || "Internal server error", code: err.code }); }
});
...
router.post('/submit/:student_id', verify('student', 'admin'), async (req, res) => {
  ...
  try {
    ...
    if (toInsert.length > 0) {
      // Include semester + academic_year_id for the CHECK constraint on new rows
      ...
      const values = toInsert.flatMap(e => [
        ...
        sem, student.academic_year_id || null
      ]);
      await db.query(
        `INSERT INTO student_subject_enrollment
           (student_id, subject_id, status, is_major, remarks, is_draft, semester, academic_year_id)
   ...
   // Mark student enrollment as submitted
    await db.query('UPDATE students SET enrollment_submitted = 1, enrollment_submitted_at = NOW() WHERE student_id = ?', [req.params.student_id]);

const skipped_admin_locked = enrollments.length - toInsert.length;
    res.json({
      message: 'Enrollment submitted successfully!',
      inserted: toInsert.length,
      skipped_admin_locked,
      total_requested: enrollments.length
    });
  } catch (err) {
